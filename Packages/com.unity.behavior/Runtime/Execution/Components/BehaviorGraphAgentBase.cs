...
{
    public abstract class BehaviorGraphAgentBase : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public bool IsStarted { get => m_IsStarted; set => m_IsStarted = value; }
        public bool IsInitialised { get => m_IsInitialised; set => m_IsInitialised = value; }
        public bool IsRunning { get => m_IsRunning; set => m_IsRunning = value; }
        
        [SerializeReference, HideInInspector]
        protected List<BlackboardVariable> m_BlackboardVariableOverridesList = new ();
        ...
        public void Init()
        {
            ...
            m_Graph = GetBehaviorGraphInstance();
            m_Graph.AssignGameObjectToGraphModules(gameObject);
            InitChannelsAndMetadata();
            m_IsInitialised = true;
        }

        protected virtual BehaviorGraph GetBehaviorGraphInstance() {
            return m_Graph;
        }
        ...
        public virtual void RestartGraph() {
            if (m_Graph == null)
            {
        ...
        protected virtual void OnAssignBehaviorGraph()
        {
            // If the graph or blackboard are null, we can't sync the overrides, so return.
            if (Graph == null || Graph.BlackboardReference?.Blackboard == null)
            {
        ...
        protected void InitChannelsAndMetadata(bool applyOverride = true)
        {
            if (applyOverride)
            {
        ...
        private bool TrySetBlackboardVariableOverride<TValue>(string variableName, TValue value)
        {
            ...
            var candidate = Graph.BlackboardReference.Blackboard.Variables.Where(bbVariable => bbVariable.Name == variableName).First();
        ...
        private bool TrySetBlackboardVariableOverride<TValue>(SerializableGUID guid, TValue value)
        {
            ...
            // We check for the source blackboard and override it if needed.
            // TODO: If we want to support override of linked blackboard asset in the future, we will need to also add support for it here.
            var candidate = Graph.BlackboardReference.Blackboard.Variables.Where(bbVariable => bbVariable.GUID == guid).First();
            if (candidate is null)
            {
                return false;
            }
        ...
        protected void CreateOrUpdateSelfOverride()
        {
        ...
        protected void RemoveOverridesWithNoMatchInBlackboard()
        {
            ...
            {
                if (Graph.BlackboardReference.Blackboard.Variables.Any(bbVariable => bbVariable.GUID == guid))
                {
                    continue;
                }
        ...
        protected void UpdateBlackboardOverridesToMatchBlackboard()
        {
            foreach (BlackboardVariable variable in Graph.BlackboardReference.Blackboard.Variables)
            {
#if UNITY_EDITOR
        ...
        protected void ApplySelfOverrideToBlackboard()
        {
            SerializableGUID graphOwnerID = BehaviorGraph.k_GraphSelfOwnerID;
            if (m_BlackboardOverrides.TryGetValue(graphOwnerID, out BlackboardVariable ownerVariableOverride))
            {
